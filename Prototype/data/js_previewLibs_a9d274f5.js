...
Request.JSON = new Class({
  ...
  MooTools.$A = $A;
  MooTools.$arguments = $arguments;
  MooTools.$H = $H;
  MooTools.$chk = $chk;
  MooTools.$clear = $clear;
  MooTools.$defined = $defined;
  MooTools.$each = $each;
  MooTools.$empty = $empty;
  MooTools.$extend = $extend;
  MooTools.$lambda = $lambda;
  MooTools.$merge = $merge;
  MooTools.$mixin = $mixin;
  MooTools.$pick = $pick;
  MooTools.$random = $random;
  MooTools.$splat = $splat;
  MooTools.$time = $time;
  MooTools.$try = $try;
  MooTools.$type = $type;
  MooTools.$unlink = $unlink;
  MooTools.Array = Array;
  MooTools.Browser = Browser;
  MooTools.Chain = Chain;
  MooTools.Class = Class;
  MooTools.Cookie = Cookie;
  MooTools.Document = Document;
  MooTools.Element = Element;
  MooTools.Elements = Elements;
  MooTools.Event = Event;
  MooTools.Events = Events;
  MooTools.Function = Function;
  MooTools.Fx = Fx;
  MooTools.Hash = Hash;
  MooTools.JSON = JSON;
  MooTools.Native = Native;
  MooTools.Number = Number;
  MooTools.Options = Options;
  MooTools.Request = Request;
  MooTools.Selectors = Selectors;
  MooTools.String = String;
  MooTools.Swiff = Swiff;
  MooTools.Window = Window;
  window.MooTools = MooTools;
...
(function(){
	...
	MooTools.lang = new MooTools.Events();

	MooTools.$extend(MooTools.lang, {
		...
		load: function() {
			...
			cascaded = {};
			MooTools.$each(langs, function(set, setName){
		...
		cascade: function(lang){
			...
			return MooTools.$merge.apply(this, langs);
		},

		lambda: function(set) {
			(set || {}).get = function(key, args){
				return MooTools.$lambda(set[key]).apply(this, MooTools.$splat(args));
			};
			return set;
		},
		...
		set: function(lang, set, members){
			...
			MooTools.$extend(langData[set], members);
		...
		list: function(){
			return MooTools.Hash.getKeys(data.languages);
		}
...
var log = function(){
	if (global.console && console.log){
		try {
			...
			console.log(MooTools.Array.slice(arguments));
		}
	} else {
...
var Depender = {
	...
	options: {
		/* 
		onRequire: MooTools.$empty(options),
		onRequirementLoaded: MooTools.$empty([scripts, options]),
		onScriptLoaded: MooTools.$empty({
			script: script, 
		...
		log: false,*/
		loadedSources: [],
		loadedScripts: ['Core', 'MooTools.Browser', 'MooTools.Array', 'MooTools.String', 'MooTools.Function', 'MooTools.Number', 'MooTools.Hash', 'MooTools.Element', 'MooTools.Event', 'MooTools.Element.Event', 'MooTools.Class', 'DomReady', 'MooTools.Class.Extras', 'MooTools.Request', 'MooTools.JSON', 'MooTools.Request.JSON', 'More', 'Depender', 'Log'],
	...
	include: function(libs){
		...
		var loader = function(data){
			this.libs = MooTools.$merge(this.libs, data);
			MooTools.$each(this.libs, function(data, lib){
				if (data.scripts) this.loadSource(lib, data.scripts);
			}, this);
		}.bind(this);
		if (MooTools.$type(libs) == 'string'){
	...
	dataLoaded: function(){
		var loaded = true;
		MooTools.$each(this.libs, function(v, k){
	...
	calculateLoaded: function(){
		...
		if (this.options.loadedSources){
			this.options.loadedSources.each(function(lib){
				MooTools.$each(this.libs[lib].files, function(dir){
					MooTools.$each(dir, function(data, file){
						set(file);
					}, this);
				}, this);
			}, this);
	...
	mapTree: function(){
		MooTools.$each(this.libs, function(data, source){
			MooTools.$each(data.files, function(scripts, folder){
				MooTools.$each(scripts, function(details, script){
	...
	calculateDependencies: function(scripts){
		var reqs = [];
		MooTools.$splat(scripts).each(function(script){
...
MooTools.$extend(Depender, new MooTools.Events);
MooTools.$extend(Depender, new MooTools.Options);
MooTools.$extend(Depender, new Log);
...
MooTools.Class.refactor = function(original, refactors){

	MooTools.$each(refactors, function(item, name){
...
MooTools.Class.Mutators.Binds = function(binds){
    return binds;
};

MooTools.Class.Mutators.initialize = function(initialize){
	return function(){
		MooTools.$splat(this.Binds).each(function(name){
...
MooTools.Class.Occlude = new MooTools.Class({

	occlude: function(property, element){
		element = document.id(element || this.element);
		var instance = element.retrieve(property || this.property);
		if (instance && !MooTools.$defined(this.occluded))
	...
	var wait = {
		wait: function(duration){
			return this.chain(function(){
				this.callChain.delay(MooTools.$pick(duration, 500), this);
			}.bind(this));
		}
	};

	MooTools.Chain.implement(wait);

	if (window.MooTools.Fx){
		MooTools.Fx.implement(wait);
		['Css', 'Tween', 'MooTools.Elements'].each(function(cls){
			if (MooTools.Fx[cls]) MooTools.Fx[cls].implement(wait);
		});
	}

	MooTools.Element.implement({
		chains: function(effects){
			MooTools.$splat(MooTools.$pick(effects, ['tween', 'morph', 'reveal'])).each(function(effect){
		...
		pauseFx: function(duration, effect){
			this.chains(effect).get(MooTools.$pick(effect, 'tween')).wait(duration);
			return this;
		}
	});
...
if (!Date.now) Date.now = MooTools.$time;
...
var pad = function(what, length){
	return new MooTools.Array(length - MooTools.String(what).length + 1).join('0') + what;
};

Date.implement({

	set: function(prop, value){
		switch (MooTools.$type(prop)){
			case 'object':
				for (var p in prop) this.set(p, prop[p]);
				break;
			case 'string':
	...
	decrement: function(interval, times){
		return this.increment(interval, -1 * MooTools.$pick(times, 1));
	},
	...
	diff: function(date, resolution){
		if (MooTools.$type(date) == 'string') date = Date.parse(date);
...
Date.extend({
	...
	units: {
		ms: MooTools.$lambda(1),
		second: MooTools.$lambda(1000),
		minute: MooTools.$lambda(60000),
		hour: MooTools.$lambda(3600000),
		day: MooTools.$lambda(86400000),
		week: MooTools.$lambda(608400000),
		month: function(month, year){
			var d = new Date;
			return Date.daysInMonth(MooTools.$pick(month, d.get('mo')), MooTools.$pick(year, d.get('year'))) * 86400000;
		},
		year: function(year){
	...
	defineParsers: function(){
		MooTools.Array.flatten(arguments).each(Date.defineParser);
	},
...
MooTools.Hash.implement({
	...
	cleanValues: function(method){
		method = method || MooTools.$defined;
	...
	run: function(){
		var args = arguments;
		this.each(function(v, k){
			if (MooTools.$type(v) == 'function') v.run(args);
		});
	}
...
script: MooTools.String.Extras.js
...
MooTools.String.implement({
	...
	repeat: function(times){
		return new MooTools.Array(times + 1).join(this);
	},
	...
	tidy: function(){
		...
		MooTools.$each(tidymap, function(value, key){
...
MooTools.String.implement({
	...
	parseQueryString: function(){
		...
		if (vars.length) vars.each(function(val){
			...
			keys.each(function(key, i){
				...
				if(i < keys.length - 1)
					obj = obj[key] = current || {};
				else if(MooTools.$type(current) == 'array')
					current.push(value);
				else
					obj[key] = MooTools.$defined(current) ? [current, value] : value;
			});
		});
		return res;
	},
	...
	cleanQueryString: function(method){
		return this.split('&').filter(function(val){
			...
			return method ? method.run([key, value]) : MooTools.$chk(value);
		}).join('&');
	}
...
var URI = new MooTools.Class({
	...
	initialize: function(uri, options){
		...
		if (uri && uri.parsed) this.parsed = MooTools.$unlink(uri.parsed);
	...
	set: function(part, value, base){
		if (part == 'value'){
			...
			if (scheme && !MooTools.$defined(this.schemes[scheme.toLowerCase()])) this.parsed = { scheme: scheme, value: value };
	...
	getData: function(key, part){
		...
		if (!MooTools.$chk(qs)) return key ? null : {};
	...
	setData: function(values, merge, part){
		if (typeof values == 'string'){
			...
			values = MooTools.$merge(this.getData(), values);
		}
		return this.set(part || 'query', MooTools.Hash.toQueryString(values));
	},
...
URI = MooTools.Class.refactor(URI, {
...
MooTools.Element.implement({
	...
	getSelectedRange: function() {
		if (MooTools.$defined(this.selectionStart)) return {start: this.selectionStart, end: this.selectionEnd};
	...
	insertAtCursor: function(value, select){
		...
		if (MooTools.$pick(select, true)) this.selectRange(pos.start, pos.start + value.length);
		else this.setCaretPosition(pos.start + value.length);
		return this;
	},

	insertAroundCursor: function(options, select){
		options = MooTools.$extend({
			before: '',
			defaultMiddle: '',
			after: ''
		}, options);
		...
		if (pos.start == pos.end){
			...
			if (MooTools.$pick(select, true)) this.selectRange(selStart, selStart + current.length);
...
MooTools.Elements.from = function(text, excludeScripts){
	if (MooTools.$pick(excludeScripts, true)) text = text.stripScripts();

	var container, match = text.match(/^\s*<(t[dhr]|tbody|tfoot|thead)/i);

	if (match){
		container = new MooTools.Element('table');
		var tag = match[1].toLowerCase();
		if (['td', 'th', 'tr'].contains(tag)){
			container = new MooTools.Element('tbody').inject(container);
			if (tag != 'tr') container = new MooTools.Element('tr').inject(container);
		}
	}

	return (container || new MooTools.Element('div')).set('html', text).getChildren();
};
		...
		check = function(e, selector){
			...
			if (combinators.test(selector = selector.trim())){
				...
				for ( ; t && t != this; t = t.parentNode){
					if (MooTools.Element.match(t, selector)) return document.id(t);
				}
			}
			return null;
		};
...
})(MooTools.Element.prototype.addEvent, MooTools.Element.prototype.removeEvent);
...
MooTools.Element.implement({
	...
	expose: function(){
		if (this.getStyle('display') != 'none') return MooTools.$empty;
	...
	getDimensions: function(options){
		options = MooTools.$merge({computeSize: false},options);
		var dim = {};
		...
		return MooTools.$chk(dim.x) ? MooTools.$extend(dim, {width: dim.x, height: dim.y}) : MooTools.$extend(dim, {x: dim.width, y: dim.height});
	},

	getComputedSize: function(options){
		options = MooTools.$merge({
		...
		MooTools.$each(options.plains, function(plain, key){
		...
		var subtracted = [];
		MooTools.$each(options.plains, function(plain, key){ //keys: width, height, plains: ['left', 'right'], ['top','bottom']
		...
		['Width', 'Height'].each(function(value){
			...
			if(!MooTools.$chk(size[lower])) return;
		...
		return MooTools.$extend(styles, size);
	}
...
(function(){
	...
	window.addEvent('domready', function(){
		var test = new MooTools.Element('div').setStyles({
			position: 'fixed',
			top: 0,
			right: 0
	...
	MooTools.Element.implement({
		...
		pin: function(enable){
			...
			if (enable !== false){
				...
				this.setStyles(MooTools.$merge(reposition, {position: 'absolute'})).removeClass('isPinned');
			}
			return this;
		},
...
var original = MooTools.Element.prototype.position;

MooTools.Element.implement({

	position: function(options){
		//call original position if the options are x/y values
		if (options && (MooTools.$defined(options.x) || MooTools.$defined(options.y))) return original ? original.apply(this, arguments) : this;
		MooTools.$each(options||{}, function(v, k){ if (!MooTools.$defined(v)) delete options[k]; });
		options = MooTools.$merge({
			// minimum: { x: 0, y: 0 },
			// maximum: { x: 0, y: 0},
		...
		var fixValue = function(option){
			if (MooTools.$type(option) != 'string') return option;
...
var OverText = new MooTools.Class({

	Implements: [MooTools.Options, MooTools.Events, MooTools.Class.Occlude],

	Binds: ['reposition', 'assert', 'focus', 'hide'],

	options: {/*
		textOverride: null,
		onFocus: MooTools.$empty()
		onTextHide: MooTools.$empty(textEl, inputEl),
		onTextShow: MooTools.$empty(textEl, inputEl), */
		element: 'label',
		positionOptions: {
			position: 'upperLeft',
			edge: 'upperLeft',
	...
	attach: function(){
		...
		if (!val) return;
		this.text = new MooTools.Element(this.options.element, {
			'class': 'overTxtLabel',
			styles: {
				lineHeight: 'normal',
				position: 'absolute',
		...
		if (this.options.wrap) {
			this.textHolder = new MooTools.Element('div', {
				styles: {
					lineHeight: 'normal',
					position: 'relative'
				},
	...
	poll: function(stop){
		...
		if (stop) MooTools.$clear(this.poller);
	...
	reposition: function(){
		...
		if (this.text && this.test()) this.text.position(MooTools.$merge(this.options.positionOptions, {relativeTo: this.element}));
		return this;
	}
...
if (window.MooTools.Fx && MooTools.Fx.Reveal) {
	MooTools.Fx.Reveal.implement({
		hideInputs: MooTools.Browser.Engine.trident ? 'select, input, textarea, object, embed, .overTxtLabel' : false
	});
}
...
MooTools.Fx.Elements = new MooTools.Class({

	Extends: MooTools.Fx.CSS,
...
MooTools.Fx.Accordion = new MooTools.Class({

	Extends: MooTools.Fx.Elements,

	options: {/*
		onActive: MooTools.$empty(toggler, section),
		onBackground: MooTools.$empty(toggler, section),
		fixedHeight: false,
		fixedWidth: false,
		*/
		display: 0,
	...
	initialize: function(){
		var params = MooTools.Array.link(arguments, {
			'container': MooTools.Element.type, //deprecated
			'options': Object.type,
			'togglers': MooTools.$defined,
			'elements': MooTools.$defined
		});
		this.parent(params.elements, params.options);
		this.togglers = $$(params.togglers);
		this.previous = -1;
		this.internalChain = new MooTools.Chain();
		if (this.options.alwaysHide) this.options.wait = true;
		if (MooTools.$chk(this.options.show)){
		...
		if (MooTools.$chk(this.options.display) || this.options.initialDisplayFx === false) this.display(this.options.display, this.options.initialDisplayFx);
	...
	display: function(index, useFx){
		...
		useFx = MooTools.$pick(useFx, true);
		...
		index = (MooTools.$type(index) == 'element') ? this.elements.indexOf(index) : index;
...
*/
var Accordion = new MooTools.Class({

	Extends: MooTools.Fx.Accordion,

	initialize: function(){
		this.parent.apply(this, arguments);
		var params = MooTools.Array.link(arguments, {'container': MooTools.Element.type});
...
MooTools.Fx.Move = new MooTools.Class({

	Extends: MooTools.Fx.Morph,
	...
	start: function(destination){
		return this.parent(this.element.position(MooTools.$merge(this.options, destination, {returnPos: true})));
	}

});

MooTools.Element.Properties.move = {

	set: function(options){
		var morph = this.retrieve('move');
		if (morph) morph.cancel();
		return this.eliminate('move').store('move:options', MooTools.$extend({link: 'cancel'}, options));
	},

	get: function(options){
		if (options || !this.retrieve('move')){
			if (options || !this.retrieve('move:options')) this.set('move', options);
			this.store('move', new MooTools.Fx.Move(this, this.retrieve('move:options')));
		}
...
MooTools.Fx.Reveal = new MooTools.Class({

	Extends: MooTools.Fx.Morph,

	options: {/*	  
		onShow: MooTools.$empty(thisElement),
		onHide: MooTools.$empty(thisElement),
		onComplete: MooTools.$empty(thisElement),
		heightOverride: null,
		widthOverride: null, */
		link: 'cancel',
		styles: ['padding', 'border', 'margin'],
		transitionOpacity: !MooTools.Browser.Engine.trident4,
		mode: 'vertical',
		display: 'block',
		hideInputs: MooTools.Browser.Engine.trident ? 'select, input, textarea, object, embed' : false
	},
		...
		try {
			if (!this.hiding && !this.showing){
				if (this.element.getStyle('display') != 'none'){
					...
					var zero = {};
					MooTools.$each(startStyles, function(style, name){
						zero[name] = [style, 0];
					}, this);
		...
		try {
			if (!this.showing && !this.hiding){
				if (this.element.getStyle('display') == 'none' ||
					...
					if (MooTools.$chk(this.options.heightOverride)) startStyles.height = this.options.heightOverride.toInt();
					if (MooTools.$chk(this.options.widthOverride)) startStyles.width = this.options.widthOverride.toInt();
					...
					MooTools.$each(startStyles, function(style, name){ zero[name] = 0; });
					//set to zero
					this.element.setStyles(MooTools.$merge(zero, {overflow: 'hidden'}));
					//hide inputs
					var hideThese = this.options.hideInputs ? this.element.getElements(this.options.hideInputs) : null;
					if (hideThese) hideThese.setStyle('visibility', 'hidden');
					//start the effect
...
MooTools.Element.Properties.reveal = {
	...
	get: function(options){
		if (options || !this.retrieve('reveal')){
			...
			this.store('reveal', new MooTools.Fx.Reveal(this, this.retrieve('reveal:options')));
		}
...
MooTools.Element.Properties.dissolve = MooTools.Element.Properties.reveal;
...
MooTools.Element.implement({
	...
	nix: function(){
		var params = MooTools.Array.link(arguments, {destroy: Boolean.type, options: Object.type});
	...
	wink: function(){
		var params = MooTools.Array.link(arguments, {duration: MooTools.Number.type, options: Object.type});
...
MooTools.Fx.Scroll = new MooTools.Class({
	...
	initialize: function(element, options){
		...
		if (MooTools.$type(this.element) != 'element') this.element = document.id(this.element.getDocument().body);
	...
	set: function(){
		var now = MooTools.Array.flatten(arguments);
		if (MooTools.Browser.Engine.gecko) now = [Math.round(now[0]), Math.round(now[1])];
		this.element.scrollTo(now[0], now[1]);
	},

	compute: function(from, to, delta){
		return [0, 1].map(function(i){
			return MooTools.Fx.compute(from[i], to[i], delta);
		});
	},
	...
	start: function(x, y){
		...
		for (var z in values){
			...
			if (MooTools.$chk(values[z])) values[z] = (MooTools.$type(values[z]) == 'number') ? values[z] : max;
	...
	scrollIntoView: function(el, axes, offset){
		axes = axes ? MooTools.$splat(axes) : ['x','y'];
		var to = {};
		el = document.id(el);
...
MooTools.Fx.Slide = new MooTools.Class({
	...
	initialize: function(element, options){
		this.addEvent('complete', function(){
			...
			if (this.open && MooTools.Browser.Engine.webkit419) this.element.dispose().inject(this.wrapper);
		}, true);
		this.element = this.subject = document.id(element);
		this.parent(options);
		var wrapper = this.element.retrieve('wrapper');
		var styles = this.element.getStyles('margin', 'position', 'overflow');
		if (this.options.hideOverflow) styles = MooTools.$extend(styles, {overflow: 'hidden'});
		if (this.options.wrapper) wrapper = document.id(this.options.wrapper).setStyles(styles);
		this.wrapper = wrapper || new MooTools.Element('div', {
			styles: styles
		}).wraps(this.element);
...
MooTools.Element.Properties.slide = {

	set: function(options){
		var slide = this.retrieve('slide');
		if (slide) slide.cancel();
		return this.eliminate('slide').store('slide:options', MooTools.$extend({link: 'cancel'}, options));
	},

	get: function(options){
		if (options || !this.retrieve('slide')){
			if (options || !this.retrieve('slide:options')) this.set('slide', options);
			this.store('slide', new MooTools.Fx.Slide(this, this.retrieve('slide:options')));
		}
...
var SmoothScroll = MooTools.Fx.SmoothScroll = new MooTools.Class({

	Extends: MooTools.Fx.Scroll,
	...
	initialize: function(options, context){
		...
		if (!MooTools.Browser.Engine.webkit419) {
			this.addEvent('complete', function(){
...
MooTools.Fx.Sort = new MooTools.Class({
	...
	sort: function(newOrder){
		if (MooTools.$type(newOrder) != 'array') return false;
		var top = 0,
			left = 0,
			next = {},
			zero = {},
	...
	swap: function(one, two){
		if (MooTools.$type(one) == 'element') one = this.elements.indexOf(one);
		if (MooTools.$type(two) == 'element') two = this.elements.indexOf(two);
...
var Drag = new MooTools.Class({

	Implements: [MooTools.Events, MooTools.Options],

	options: {/*
		onBeforeStart: MooTools.$empty(thisElement),
		onStart: MooTools.$empty(thisElement, event),
		onSnap: MooTools.$empty(thisElement)
		onDrag: MooTools.$empty(thisElement, event),
		onCancel: MooTools.$empty(thisElement),
		onComplete: MooTools.$empty(thisElement, event),*/
		snap: 6,
		unit: 'px',
		grid: false,
		style: true,
	...
	initialize: function(){
		var params = MooTools.Array.link(arguments, {'options': Object.type, 'element': MooTools.$defined});
		this.element = document.id(params.element);
		this.document = this.element.getDocument();
		this.setOptions(params.options || {});
		var htype = MooTools.$type(this.options.handle);
		this.handles = ((htype == 'array' || htype == 'collection') ? $$(this.options.handle) : document.id(this.options.handle)) || this.element;
		this.mouse = {'now': {}, 'pos': {}};
		this.value = {'start': {}, 'now': {}};

		this.selection = (MooTools.Browser.Engine.trident) ? 'selectstart' : 'mousedown';
		...
		this.bound = {
			...
			eventStop: MooTools.$lambda(false)
		};
		this.attach();
	},
	...
	start: function(event){
		...
		for (var z in this.options.modifiers){
			...
			if (limit && limit[z]){
				for (var i = 2; i--; i){
					if (MooTools.$chk(limit[z][i])) this.limit[z][i] = MooTools.$lambda(limit[z][i])();
				}
			}
		}
		if (MooTools.$type(this.options.grid) == 'number') this.options.grid = {x: this.options.grid, y: this.options.grid};
...
MooTools.Element.implement({
	...
	makeResizable: function(options){
		var drag = new Drag(this, MooTools.$merge({modifiers: {x: 'width', y: 'height'}}, options));
...
Drag.Move = new MooTools.Class({
	...
	initialize: function(element, options){
		...
		if (this.container && MooTools.$type(this.container) != 'element')
	...
	calculateLimit: function(){
		...
		if (this.element.getStyle('position') == 'relative'){
			...
			if (this.container != offsetParent){
				...
				top += (MooTools.Browser.Engine.trident4 ? 0 : containerMargin.top) + offsetParentPadding.top;
			}
		} else {
			left -= elementMargin.left;
			top -= elementMargin.top;
...
var Slider = new MooTools.Class({
	...
	options: {/*
		onTick: MooTools.$empty(intPosition),
		onChange: MooTools.$empty(intStep),
		onComplete: MooTools.$empty(strStep),*/
	...
	initialize: function(element, knob, options){
		...
		this.min = MooTools.$chk(this.options.range[0]) ? this.options.range[0] : 0;
		this.max = MooTools.$chk(this.options.range[1]) ? this.options.range[1] : this.options.steps;
...
var Sortables = new MooTools.Class({

	Implements: [MooTools.Events, MooTools.Options],

	options: {/*
		onSort: MooTools.$empty(element, clone),
		onStart: MooTools.$empty(element, clone),
		onComplete: MooTools.$empty(element),*/
		snap: 4,
		opacity: 1,
		clone: false,
		revert: false,
	...
	initialize: function(lists, options){
		...
		if (this.options.revert) this.effect = new MooTools.Fx.Morph(null, MooTools.$merge({duration: 250, link: 'cancel'}, this.options.revert));
	},
	...
	addItems: function(){
		MooTools.Array.flatten(arguments).each(function(element){
	...
	addLists: function(){
		MooTools.Array.flatten(arguments).each(function(list){
	...
	removeItems: function(){
		return $$(MooTools.Array.flatten(arguments).map(function(element){
	...
	removeLists: function(){
		return $$(MooTools.Array.flatten(arguments).map(function(list){
	...
	getClone: function(event, element){
		if (!this.options.clone) return new MooTools.Element('div').inject(document.body);
		if (MooTools.$type(this.options.clone) == 'function') return this.options.clone.call(this, event, element, this.list);
	...
	serialize: function(){
		var params = MooTools.Array.link(arguments, {modifier: MooTools.Function.type, index: MooTools.$defined});
		...
		return MooTools.$chk(index) && index >= 0 && index < this.lists.length ? serial[index] : serial;
	}
...
MooTools.Request.JSONP = new MooTools.Class({

	Implements: [MooTools.Chain, MooTools.Events, MooTools.Options, Log],

	options: {/*
		onRetry: MooTools.$empty(intRetries),
		onRequest: MooTools.$empty(scriptElement),
		onComplete: MooTools.$empty(data),
		onSuccess: MooTools.$empty(data),
	...
	send: function(options){
		if (!MooTools.$chk(arguments[1]) && !this.check(options)) return this;

		var type = MooTools.$type(options), 
				old = this.options, 
				index = MooTools.$chk(arguments[1]) ? arguments[1] : this.requests++;
		if (type == 'string' || type == 'element') options = {data: options};

		options = MooTools.$extend({data: old.data, url: old.url}, options);

		if (!MooTools.$chk(this.triesRemaining[index])) this.triesRemaining[index] = this.options.retries;
		...
		}).delay(MooTools.Browser.Engine.trident ? 50 : 0, this);
		return this;
	},
	...
	getScript: function(options){
		var index = MooTools.Request.JSONP.counter,
				data;
		MooTools.Request.JSONP.counter++;

		switch (MooTools.$type(options.data)){
			case 'element': data = document.id(options.data).toQueryString(); break;
			case 'object': case 'hash': data = MooTools.Hash.toQueryString(options.data);
		}

		var src = options.url + 
			 (options.url.test('\\?') ? '&' :'?') + 
			 (options.callbackKey || this.options.callbackKey) + 
			 '=MooTools.Request.JSONP.request_map.request_'+ index + 
			 (data ? '&' + data : '');
		if (src.length > 2083) this.log('JSONP '+ src +' will fail in Internet Explorer, which enforces a 2083 bytes length limit on URIs');

		var script = new MooTools.Element('script', {type: 'text/javascript', src: src});
		MooTools.Request.JSONP.request_map['request_' + index] = function(){ this.success(arguments, script); }.bind(this);
...
MooTools.Request.JSONP.counter = 0;
MooTools.Request.JSONP.request_map = {};
...
MooTools.Request.Queue = new MooTools.Class({
	...
	options: {/*
		onRequest: MooTools.$empty(argsPassedToOnRequest),
		onSuccess: MooTools.$empty(argsPassedToOnSuccess),
		onComplete: MooTools.$empty(argsPassedToOnComplete),
		onCancel: MooTools.$empty(argsPassedToOnCancel),
		onException: MooTools.$empty(argsPassedToOnException),
		onFailure: MooTools.$empty(argsPassedToOnFailure),
		onEnd: MooTools.$empty,
		*/
		stopOnFailure: true,
		autoAdvance: true,
		concurrent: 1,
	...
	initialize: function(options){
		...
		this.setOptions(options);
		this.requests = new MooTools.Hash;
		this.queue = [];
		this.reqBinders = {};
	...
	addRequests: function(obj){
		MooTools.$each(obj, function(req, name){
	...
	removeRequest: function(req){
		var name = MooTools.$type(req) == 'object' ? this.getName(req) : req;
		if (!name && MooTools.$type(name) != 'string') return this;
...
var Asset = {
	...
	javascript: function(source, properties){
		properties = MooTools.$extend({
			onload: MooTools.$empty,
			document: document,
			check: MooTools.$lambda(true)
		}, properties);
		
		if (properties.onLoad) properties.onload = properties.onLoad;
		
		var script = new MooTools.Element('script', {src: source, type: 'text/javascript'});
		...
		if (MooTools.Browser.Engine.webkit419) var checker = (function(){
			if (!MooTools.$try(check)) return;
			MooTools.$clear(checker);
			load();
		}).periodical(50);
	...
	css: function(source, properties){
		return new MooTools.Element('link', MooTools.$merge({
			rel: 'stylesheet',
			media: 'screen',
			type: 'text/css',
			href: source
	...
	image: function(source, properties){
		properties = MooTools.$merge({
			onload: MooTools.$empty,
			onabort: MooTools.$empty,
			onerror: MooTools.$empty
		}, properties);
		var image = new Image();
		var element = document.id(image) || new MooTools.Element('img');
	...
	images: function(sources, options){
		...
		sources = MooTools.$splat(sources);
		var images = [];
		var counter = 0;
		return new MooTools.Elements(sources.map(function(source){
			return Asset.image(source, MooTools.$extend(options.properties, {
				onload: function(){
    ...
    if(color ==  "#NaNNaNNaN" || !color) {
      color = "#000000";
		...
		if (arguments.length >= 3){
			type = 'rgb'; color = MooTools.Array.slice(arguments, 0, 3);
		...
		return MooTools.$extend(color, this);
	}
...
Color.implement({
	...
	mix: function(){
		var colors = MooTools.Array.slice(arguments);
		var alpha = (MooTools.$type(colors.getLast()) == 'number') ? colors.pop() : 50;
		var rgb = this.slice();
...
var Group = new MooTools.Class({

	initialize: function(){
		this.instances = MooTools.Array.flatten(arguments);
		this.events = {};
		this.checker = {};
	},
...
MooTools.Hash.Cookie = new MooTools.Class({

	Extends: MooTools.Cookie,
	...
	save: function(){
		var value = MooTools.JSON.encode(this.hash);
	...
	load: function(){
		this.hash = new MooTools.Hash(MooTools.JSON.decode(this.read(), true));
		return this;
	}

});

MooTools.Hash.each(MooTools.Hash.prototype, function(method, name){
	if (typeof method == 'function') MooTools.Hash.Cookie.implement(name, function(){
...
var IframeShim = new MooTools.Class({
	...
	options: {
		...
		margin: 0,
		offset: {x: 0, y: 0},
		browsers: (MooTools.Browser.Engine.trident4 || (MooTools.Browser.Engine.gecko && !MooTools.Browser.Engine.gecko19 && MooTools.Browser.Platform.mac))
	},
	...
	makeShim: function(){
		if(this.options.browsers){
			...
			zIndex = (MooTools.$chk(this.options.zIndex) && zIndex > this.options.zIndex) ? this.options.zIndex : zIndex - 1;
			if (zIndex < 0) zIndex = 1;
			this.shim = new MooTools.Element('iframe', {
			...
			this.position = this.hide = this.show = this.dispose = MooTools.$lambda(this);
		}
	},
...
(function(){
	...
	var Keyboard = this.Keyboard = new MooTools.Class({

		Extends: MooTools.Events,

		Implements: [MooTools.Options, Log],

		options: {
			/*
			onActivate: MooTools.$empty,
			onDeactivate: MooTools.$empty,
			*/
	...
	MooTools.Event.Keys.extend({
		...
		'=': 187,
		',': 188,
		'-': MooTools.Browser.Engine.Gecko ? 109 : 189,
		'.': 190,
		'/': 191,
		'`': 192,
		'[': 219,
...
Keyboard.implement({
	...
	*/
	addShortcut: function(name, shortcut) {
		...
		shortcut.getKeyboard = MooTools.$lambda(this);
		shortcut.name = name;
...
Keyboard.rebind = function(newKeys, shortcuts){
	MooTools.$splat(shortcuts).each(function(shortcut){
...
var Mask = new MooTools.Class({
	...
	options: {
		// onShow: MooTools.$empty,
		// onHide: MooTools.$empty,
		// onDestroy: MooTools.$empty,
		// onClick: MooTools.$empty,
		//inject: {
		//  where: 'after',
		//  target: null,
		//},
	...
	render: function() {
		this.element = new MooTools.Element('div', {
			'class': this.options['class'],
			id: this.options.id || 'mask-' + MooTools.$time(),
			styles: MooTools.$merge(this.options.style, {
				display: 'none'
			}),
			events: {
				click: function(){
	...
	resize: function(x, y){
		...
		this.element.setStyles({
			width: MooTools.$pick(x, dim.totalWidth, dim.x),
			height: MooTools.$pick(y, dim.totalHeight, dim.y)
		});
		return this;
	},
...
MooTools.Element.Properties.mask = {
...
var Scroller = new MooTools.Class({
	...
	initialize: function(element, options){
		...
		this.listener = (MooTools.$type(this.element) != 'element') ?  this.docBody : this.element;
		this.timer = null;
		this.bound = {
	...
	stop: function(){
		...
		this.detach();
		this.timer = MooTools.$clear(this.timer);
	},
...
var read = function(option, element){
	return (option) ? (MooTools.$type(option) == 'function' ? option(element) : element.get(option)) : '';
};

this.Tips = new MooTools.Class({

	Implements: [MooTools.Events, MooTools.Options],

	options: {
		/*
		onAttach: MooTools.$empty(element),
		onDetach: MooTools.$empty(element),
		*/
		onShow: function(){
	...
	initialize: function(){
		var params = MooTools.Array.link(arguments, {options: Object.type, elements: MooTools.$defined});
		this.setOptions(params.options);
		if (params.elements) this.attach(params.elements);
		this.container = new MooTools.Element('div', {'class': 'tip'});
	},

	toElement: function(){
		if (this.tip) return this.tip;

		return this.tip = new MooTools.Element('div', {
			'class': this.options.className,
			styles: {
				position: 'absolute',
				top: 0,
			...
			new MooTools.Element('div', {'class': 'tip-top'}),
			this.container,
			new MooTools.Element('div', {'class': 'tip-bottom'})
		).inject(document.body);
	},
	...
	elementEnter: function(event, element){
		...
		['title', 'text'].each(function(value){
			...
			if (content) this.fill(new MooTools.Element('div', {'class': 'tip-' + value}).inject(this.container), content);
		}, this);
...
var Spinner = new MooTools.Class({
	...
	render: function(){
		this.parent();
		this.element.set('id', this.options.id || 'spinner-'+MooTools.$time());
		this.content = document.id(this.options.content) || new MooTools.Element('div', this.options.content);
		this.content.inject(this.element);
		if (this.options.message) {
			this.msg = document.id(this.options.message) || new MooTools.Element('p', this.options.messageContainer).appendText(this.options.message);
			this.msg.inject(this.content);
		}
		if (this.options.img) {
			this.img = document.id(this.options.img) || new MooTools.Element('div', this.options.img);
	...
	showMask: function(noFx){
		var pos = function(){
			this.content.position(MooTools.$merge({
				relativeTo: this.element
...
Spinner.implement(new MooTools.Chain);

if (window.MooTools.Request) {
	MooTools.Request = MooTools.Class.refactor(MooTools.Request, {
...
MooTools.Element.Properties.spinner = {
...
MooTools.Element.implement({
	...
	unspin: function(){
		var opt = MooTools.Array.link(arguments, {options: Object.type, callback: MooTools.Function.type});
...
MooTools.Element.implement({
 disableSelection: function() {
  if(MooTools.Browser.Engine.trident || MooTools.Browser.Engine.webkit) {
   this.onselectstart = function() { return false; }
   if (MooTools.Browser.Engine.webkit) {
     this.setStyle('-webkit-user-select', 'none');
   }
  }
  else if(MooTools.Browser.Engine.gecko) {
...
document.blur = (function() {
  ...
  return function() {
    ...
    if(MooTools.Browser.Engine.webkit && MooTools.Browser.Platform.mac) {
      tmp.blur();
...
sprintfWrapper = {
	init : function () {
		...
		var matches = new MooTools.Array();
		var strings = new MooTools.Array();
		var convCount = -1;
		...
		while (match = exp.exec(string)) {
			...
			matches[matches.length] = {
				...
				argument: MooTools.String(arguments[convCount])
			};
		}
		...
		for (i=0; i<matches.length; i++) {
			...
			if (matches[i].code == '%') { substitution = '%' }
				...
				matches[i].argument = MooTools.String(Math.abs(parseInt(matches[i].argument)).toString(2));
				substitution = sprintfWrapper.convert(matches[i], true);
			}
			else if (matches[i].code == 'c') {
				matches[i].argument = MooTools.String(MooTools.String.fromCharCode(parseInt(Math.abs(parseInt(matches[i].argument)))));
				substitution = sprintfWrapper.convert(matches[i], true);
			}
			else if (matches[i].code == 'd') {
				matches[i].argument = MooTools.String(Math.abs(parseInt(matches[i].argument)));
				substitution = sprintfWrapper.convert(matches[i]);
			}
			else if (matches[i].code == 'f') {
				matches[i].argument = MooTools.String(Math.abs(parseFloat(matches[i].argument)).toFixed(matches[i].precision ? matches[i].precision : 6));
				substitution = sprintfWrapper.convert(matches[i]);
			}
			else if (matches[i].code == 'o') {
				matches[i].argument = MooTools.String(Math.abs(parseInt(matches[i].argument)).toString(8));
				substitution = sprintfWrapper.convert(matches[i]);
			}
				...
				matches[i].argument = MooTools.String(Math.abs(parseInt(matches[i].argument)).toString(16));
				substitution = sprintfWrapper.convert(matches[i]);
			}
			else if (matches[i].code == 'X') {
				matches[i].argument = MooTools.String(Math.abs(parseInt(matches[i].argument)).toString(16));
	...
	convert : function(match, nosign){
		...
		var pad = new MooTools.Array(l < 0 ? 0 : l).join(match.pad);
		if (!match.left) {
...
MooTools.String.prototype.sprintf = sprintfWrapper.init;

MooTools.String.prototype.repeat = function(l){
  return new MooTools.Array(l+1).join(this);
}

MooTools.String.prototype.escapeHTML = function() {
  return this.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
}

MooTools.String.prototype.unescapeHTML = function() {
  return this.replace(/&quot;/g, '"').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
}

MooTools.Number.prototype.toIntZero = MooTools.String.prototype.toIntZero = function(){
  var v = this.toInt();
  return isNaN(v) ? 0 : v;
}

MooTools.Number.prototype.toFloatZero = MooTools.String.prototype.toFloatZero = function(){
  var v = this.toFloat();
  return isNaN(v) ? 0 : v;
}
...
MooTools.Element.implement({
  'storeEvent': function(pEvent) {
    ...
    stored_events[pEvent] = new MooTools.Element('div');
...
// ---------------------------------------------------------------------
var gSafeOnload = new MooTools.Array(); 
var gSafeOnloadDelay = new MooTools.Array();
...
{ 
  ...
  for (xyzii = 0; xyzii < xyziilen; xyzii++) {
    ...
    if (typeof(gSafeOnload[xyzii]) != 'function') {
      gSafeOnload[xyzii] = new MooTools.Function(gSafeOnload[xyzii]);
    }
...
// ---------------------------------------------------------------------
var gLoadedScripts = new MooTools.Array();
...
function AddEvent(pObject, pHandler, pFunction){
  ...
  if (document.all && document.getElementById){
    pObject[pHandler.toLowerCase()] = new MooTools.Function(pFunction);
  }
}
...
function ImgAlpha(pSrc, pWidth, pHeight, pIEScalingOff) {
  ...
  var img_el = new MooTools.Element('img');
  img_el.width = pWidth;
...
copyObject = function(pObject) {
  var newObj = MooTools.$type(pObject) == 'array' ? [] : {};
  for (i in pObject){
    if (pObject[i] && MooTools.$type(pObject[i]) == "object") {
...
var customUploader = new MooTools.Class({
  ...
  'setBrowserDifferencePosition': function(){
  	if(MooTools.Browser.Engine.gecko){
  	  this.mBrowserButtonPosition = 180;
  	}
  	else if(MooTools.Browser.Engine.presto){
...
var MD5 = function (string) {
	...
	function ConvertToWordArray(string) {
		...
		var lWordArray=MooTools.Array(lNumberOfWords-1);
		var lBytePosition = 0;
		var lByteCount = 0;
	...
	function Utf8Encode(string) {
		...
		for (var n = 0; n < string.length; n++) {
			...
			if (c < 128) {
				utftext += MooTools.String.fromCharCode(c);
			}
			else if((c > 127) && (c < 2048)) {
				utftext += MooTools.String.fromCharCode((c >> 6) | 192);
				utftext += MooTools.String.fromCharCode((c & 63) | 128);
			}
			else {
				utftext += MooTools.String.fromCharCode((c >> 12) | 224);
				utftext += MooTools.String.fromCharCode(((c >> 6) & 63) | 128);
				utftext += MooTools.String.fromCharCode((c & 63) | 128);
			}
...
MooTools.Request.Comet = new MooTools.Class({
    ...
    this.type = (MooTools.Browser.Engine.trident ? 3 : 1);
    ...
    // Firefox i inne
  		...
  		this.tunnel = new MooTools.Request.Comet.XHR(this.options);
  ...
  connectionTimeout: function() {
    ...
    if (this.type == 3) {
      ...
      this.tunnel.body.innerHTML = '<iframe src="' + this.options.url + '?json=' + encodeURIComponent(MooTools.JSON.encode(pData)) + '"></iframe>';
      this.addEvent('onPing', this.clearConnectionTimeout.bind(this));
    }
    // Opera
    else if (this.type == 2) {
      this.tunnel.setAttribute("src", this.options.url + '?json=' + encodeURIComponent(MooTools.JSON.encode(pData)));
    ...
    // Inne
      ...
      this.tunnel.post({'json': MooTools.JSON.encode(pData)});
    }
	...
	onChange: function(pResponse) {
	  ...
	  if (this.type == 2) {
      this.cancel();
      this.fireEvent('push', MooTools.JSON.decode(pResponse.data, true));
      this.fireEvent('end');
      return;
	  }
    ...
    this.cancel();
	  this.fireEvent('push', MooTools.JSON.decode(pResponse, true));
    this.fireEvent('end');
	}
});

MooTools.Request.Comet.XHR = new MooTools.Class({
	Extends: MooTools.Request,
	...
	addCallback: function(fn, options){
		...
		this.callbacks.push(MooTools.$extend(options, {fn: fn}));
		return this;
	},
 ...
 * @type {Class}
 */
var DPComet = new MooTools.Class({
  ...
  */
  ...
  setNewThreadId: function(){
    return window.name = MooTools.$random(0, 1000000) + new Date().getTime().toString().substr(4);
  },
  ...
  buildComet: function() {
    ...
    this.mComet = new MooTools.Request.Comet({
  ...
  onPush: function(pData) {
    ...
    if (pData.error) {
      ...
      DSWindow.Alert(
        'You\'ve been logged out because you\'ve logged in from another browser or you haven\'t been using UXPin for a while.',
        'Warning',
        'Go',
        10,
    ...
    new MooTools.Hash(pData).each(function(pData, pEvent) {
  ...
  onEnd: function() {
    ...
    if(200 !== response_status) {
      ...
      try {
        MooTools.JSON.decode(response_text);
      }
      catch(e) {
        delay = 15000;
      }
...
//    this.options.debugDelay = this.runDebug.delay(this.mTTL, this, [data]);
    this.mRh.post({'json': MooTools.JSON.encode(this.options.data)});
	},
...
if (MooTools.Browser.Engine.trident){
	MooTools.Element.Constructors.canvas = function(props){
...
var Canvas = new MooTools.Class({

	initialize: function(){
		var params = MooTools.Array.link(arguments, {properties: Object.type, element: MooTools.$defined});
		var props = MooTools.$extend({width: 300, height: 150}, params.properties);
...
*/
var CanvasRenderingContext2D = new MooTools.Class({

	initialize: function(el){
		this.element = new MooTools.Element('div').setStyles({
...
CanvasRenderingContext2D.implement({
	...
	*/
	isPointInPath: MooTools.Function.empty,
...
CanvasRenderingContext2D.implement({
	...
	*/
	drawImage: function (image){
		...
		var syh = sy / h, sxw = sx / w,
			m = this.m,
			Z = this.Z,
			d = MooTools.$H(this.getCoords(dx, dy)).map(function(val){ return (val / Z).round(); });
	...
	drawImageFromRect: MooTools.Function.empty,
	...
	Property: getImageData
		...
		black.
	*/
	getImageData: MooTools.Function.empty,
	...
	*/
	putImageData: MooTools.Function.empty
...
CanvasRenderingContext2D.implement({
	...
	*/
	createRadialGradient: function(x0, y0, r0, x1, y1, r1){
		return MooTools.$extend(new CanvasGradient(x0, y0, x1, y1, this), {
			r0: r0,
			r1: r1
		});
	}
...
*/
var CanvasGradient = new MooTools.Class({
...
*/
var CanvasPattern = new MooTools.Class({
...
;(function UXPConnection_Scope(global) {
  ...
  var UXPConnection = new MooTools.Class({
    ...
    observe: function() {
      ...
      var stored = lscache.get(LSKEY);

      while (stored && typeof stored === 'string') {
        try {
    ...
    isConnectionAlive: function(pCallbacks, pPeriod) {
      var callbacks = new MooTools.Hash({
        'onTrue': MooTools.$empty,
        'onFalse': MooTools.$empty
      }).extend(pCallbacks || {});
    ...
    test: function(pCallbacks) {
      var callbacks = new MooTools.Hash({'onTrue': MooTools.$empty, 'onFalse': MooTools.$empty}).extend(pCallbacks || {});
...
var StickyNotes = new MooTools.Class({
  ...
  initialize: function(pWidth, pHeight){
    ...
    this.mContainerElement = new MooTools.Element('div', {'class': 'StickyNote'});
    this.mCanvasElement.injectInside(this.mContainerElement);
    this.mTextElement = new MooTools.Element('span').injectInside(this.mContainerElement);
...
var MooTreeControl = new MooTools.Class({

  Implements: [MooTools.Options, MooTools.Events],

    options:{
        onExpand: MooTools.$empty,
        onSelect: MooTools.$empty,
        onClick: MooTools.$empty,
        onReplace: MooTools.$empty,
        onAddComponent: MooTools.$empty,
        onNodeDrop: MooTools.$empty,
        theme : 'mootree.png',
        draggable: false
    },
    ...
    initialize: function(options) {
      UXPStack.add('MooTreeControl.initialize', [options]);
        ...
        var nodeOptions = MooTools.$extend(options.nodeOptions,{'div' : this.div})
    ...
    */
    ...
    keyNavigation: function(){
        this.div.addEvent('keydown',function(event){
            var event=new MooTools.Event(event);
            if(!this.scrolled){
                if(!this.selected){
    ...
    chkOverflow: function(){//still there is some issues
        ...
        if( hide.length > 0 ){
            this.scrolled=true;
            var scroll=new MooTools.Fx.Scroll(this.div,{
                overflown: [this.div],
    ...
    DDinjectEvent: function(where,target){
        ...
        switch(where){
            ...
            case 'after' :
                ...
                this.DDinjector=new MooTools.Element('div',{'styles':
                    {
                    position:'absolute',
                    left:0,
                    width:18,
    ...
    DDclean: function(){
        ...
        if(this.DDopenTimer){
            MooTools.$clear(this.DDopenTimer);
            this.DDopenTimer=false;
        }
    },
...
var MooTreeNode = new MooTools.Class({
  ...
  Implements: [MooTools.Options, MooTools.Events],
    ...
    initialize: function(options, control) {
      UXPStack.add('MooTreeNode.initialize', [options, control]);
        ...
        this.nodes = new MooTools.Array();       // subnodes nested beneath this node (MooTreeNode objects)
        ...
        this.divs = {
            main: new MooTools.Element('div').addClass('mooTree_node').setStyle('position','relative'),
            indent: new MooTools.Element('div').setStyle('position','absolute'),
            gadget: new MooTools.Element('div'),
            node: new MooTools.Element('div'),
            icon: new MooTools.Element('div'),
            text: new MooTools.Element('div').addClass('mooTree_text'),
            sub: new MooTools.Element('div')
        }
    ...
    */
    ...
    updateText: function(){
        ...
        if (this.divs.text.style.position !== 'absolute') {
          ...
          this.divs.textsub = new MooTools.Element('div').setStyles({
            position:'relative',
            'white-space':'nowrap',
            bottom:-3,
            height:'100%',
    ...
    */
    ...
    getImg: function(name, div) {
        ...
        div = ( div || new MooTools.Element('div') ) // if no div was specified, create a new one
        ...
        for (var i=0; i<n.length; i++) if (n[i].tagName == 'node') {
            ...
            if (node.data.load) {
                ...
                node.onExpand = function(state) {
                    ...
                    this.onExpand = new MooTools.Function();
                }
            }
 ...
 */
var AutoGrow = new MooTools.Class({
  ...
  initialize: function(textarea, options) {
    ...
    this.dummy =  new MooTools.Element("div", {
      styles:	{
        "overflow-x" : "hidden",
        "position"   : "absolute",
        "top"        : 0,
...
;(function dmsDPElement_Preview_Navigation_ItemScope(global) {
  ...
  dmsDPElement_Preview_Navigation_Item.prototype.setWidth = function() {
    ...
    MooTools.Array.prototype.forEach.call(this.mEl.querySelectorAll('li span'), function(pEl) {
...
/* globals dmsDPElement_Preview_Navigation_Item */
...
;(function dmsDPElement_Preview_NavigationScope(global) {
  ...
  var dmsDPElement_Preview_Navigation = function(pEl, pType) {
    ...
    MooTools.Array.prototype.forEach.call(pEl.querySelectorAll('li > ul'), function(pChildListEl) {
      var type = null;
  ...
  dmsDPElement_Preview_Navigation.prototype.setBgColors = function(pActive, pInactive) {
    ...
    MooTools.Array.prototype.forEach.call(this.mEl.querySelectorAll('li.inactive.main'), function(pEl) {
